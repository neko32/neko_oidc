use neko_oidc::auth::codes::{PgAuthCodeStore, AuthCodeStore};
use sqlx::PgPool;
use uuid::Uuid;
...
async fn main() {
    ...
    let store = PgAuthCodeStore(pool.clone());
    let code = store.insert_auth_code(&client_id, &user_id, &redirect_uris).await.unwrap();
    println!("Code: {}", code);
}
